...
pub async fn sync_players(pool: web::Data<PgPool>) -> HttpResponse {
    match sync_fpl_players(pool.get_ref()).await {
        Ok(players) => {
let res = response_helper::success("Players Synced Successfully",
 players);
 HttpResponse::Ok().json(res)
        }
        ...
        Err(e) => {
            ...
            let res = response_helper::failure("Player sync failed",
             400, None::<Vec<FPLPlayer>>);
             HttpResponse::InternalServerError().json(res)
...
async fn sync_fpl_players(pool: &PgPool) -> Result<Vec<FPLPlayer>> {
    ...
    let client = HttpClient::new();
    let res = client.get::<FplBootstrapResponsePlayers>("https://fantasy.premierleague.com/api/bootstrap-static"). 
    await?;
